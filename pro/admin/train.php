...
if (isset($_POST['del_train'])) {
    $con = connect();
    $train_id = $con->real_escape_string($_POST['del_train']);

    // Delete dependent records in the schedule table first
    $delete_schedule = $con->query("DELETE FROM schedule WHERE train_id = '$train_id'");

    if (!$delete_schedule) {
        // Handle the error, display a message, or log it
        echo "Error deleting schedule records: " . $con->error;
    }

    // Now delete the record from the train table
    $delete_train = $con->query("DELETE FROM train WHERE id = '$train_id'");
    ...
    if ($delete_train) {
        ...
        // Handle the error, display a message, or log it
        echo "Error deleting train record: " . $con->error;
    }
}
